...
class ControllerNode(Node):
    ...
    def __init__(self):
        ...
        # config_path = self.get_parameter('vehicle_config')
        # print(config_path)
        config_path = "/home/mitch/osu-uwrt/riptide_software_ros2/src/riptide_descriptions2/config/puddles.yaml"
        ...
        self.forcePub = self.create_publisher(Twist, "{}net_force".format(self.get_namespace()), qos_profile_system_default) # TODO: get this to queue size = 1
        self.steadyPub = self.create_publisher(Bool, "{}steady".format(self.get_namespace()), qos_profile_system_default) # TODO: get this to queue size = 1
        self.accelPub = self.create_publisher(Twist, "{}~requested_accel".format(self.get_namespace()), qos_profile_system_default) # TODO: get this to queue size = 1
        self.create_subscription(Odometry, "{}odometry/filtered".format(self.get_namespace()), self.updateState, qos_profile_system_default)
        self.create_subscription(Quaternion, "{}orientation".format(self.get_namespace()), self.angularController.setTargetPosition, qos_profile_system_default)
        self.create_subscription(Vector3, "{}angular_velocity".format(self.get_namespace()), self.angularController.setTargetVelocity, qos_profile_system_default)
        self.create_subscription(Vector3, "{}position".format(self.get_namespace()), self.linearController.setTargetPosition, qos_profile_system_default)
        self.create_subscription(Vector3, "{}linear_velocity".format(self.get_namespace()), self.linearController.setTargetVelocity, qos_profile_system_default)
        self.create_subscription(Empty, "{}disable_linear".format(self.get_namespace()), self.linearController.disable, qos_profile_system_default)
        self.create_subscription(Empty, "{}off".format(self.get_namespace()), self.turnOff, qos_profile_system_default)
        self.create_subscription(Bool, "{}state/kill_switch".format(self.get_namespace()), self.switch_cb, qos_profile_system_default)        
